...
const processCommand = (cmd: string): ConsoleLine[] => {
    ...
    switch (verb) {
        ...
        case 'users':
            return [
                ...
                { id: id + 3, type: 'output', text: '  CR7729010  Kwame Asante      Active  $8,700.30', ts: now() },
                {
                    id: id + 4,
                    type: 'output',
                    text: '  VRTC9001   Sarah Lin         Active  $10,000.00 [Demo]',
                    ts: now(),
                },
        ...
        case 'balance':
            ...
            return [
                { id, type: 'info', text: `Querying balance for ${parts[1]}…`, ts: now() },
                {
                    id: id + 1,
                    type: 'success',
                    text: `  Balance: $${(Math.random() * 10000).toFixed(2)} USD`,
                    ts: now(),
                },
            ];
        case 'block':
            ...
            return [
                {
                    id,
                    type: 'error',
                    text: `⚠ Account ${parts[1]} has been BLOCKED. All sessions terminated.`,
                    ts: now(),
                },
            ];
        case 'unblock':
        ...
        case 'trade':
            if (parts.length < 4)
                return [{ id, type: 'error', text: 'Usage: trade <asset> <stake> <even|odd|over|under>', ts: now() }];
            return [
                {
                    id,
                    type: 'info',
                    text: `Executing trade: ${parts[2]} ${parts[1]} on ${parts[3].toUpperCase()}…`,
                    ts: now(),
                },
                {
                    id: id + 1,
                    type: 'success',
                    text: `  Contract ID: TRD${Math.floor(Math.random() * 100000)} — Status: OPEN`,
                    ts: now(),
                },
                {
                    id: id + 2,
                    type: 'info',
                    text: `  Expiry: 1 tick  Stake: $${parts[2]}  Payout: $${(parseFloat(parts[2]) * 1.95).toFixed(2)}`,
                    ts: now(),
                },
            ];
        case 'clear':
            ...
            return [
                {
                    id,
                    type: 'error',
                    text: `Command not found: '${verb}'. Type 'help' for available commands.`,
                    ts: now(),
                },
            ];
    }
