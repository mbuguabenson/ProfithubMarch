            ...
            <nav className='nav-items'>
                {navItems.map(item => (
                    ...
                    >
                        ...
                        {is_sidebar_open && <span className='label'>{item.label}</span>}

                        {/* Status Dots (Simulated) */}
                        {item.id === 'notifications' && <span className='status-dot'></span>}
            ...
            <div className='bottom-section'>
                {is_sidebar_open ? (
                    <div className='update-badge'>
                        ...
                        <button className='update-btn'>UPDATE SYSTEM</button>
                    </div>
                ) : (
                    <button className='toggle-btn mb-4'>🚀</button>
                )}

                <button onClick={() => admin.toggleSidebar()} className='toggle-btn'>
                    <span className={classNames('arrow', is_sidebar_open && 'flipped')}>❯</span>
                </button>
            </div>
        </aside>
    );
