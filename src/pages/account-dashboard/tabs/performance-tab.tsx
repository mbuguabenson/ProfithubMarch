    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className="glass-card tooltip-card" style={{ padding: '1rem', minWidth: '140px' }}>
                ...
                <p className={`font-black text-lg ${val >= 0 ? 'up' : 'down'}`} style={{ color: val >= 0 ? '#34d399' : '#fb7185' }}>
    ...
    return (
        <div className="account-tab-wrapper">
            <div className="grid-4">
                {[
                    { label: 'Net Alpha P&L', value: `+$${totalPnl.toLocaleString()}`, color: 'emerald', icon: TrendingUp as any },
                    ...
                    { label: 'Peak Alpha', value: `+$${bestDay.toLocaleString()}`, color: 'purple', icon: Calendar as any },
                ].map((kpi, i) => {
                    const Icon = kpi.icon;
                    return (
            ...
            <div className="glass-card chart-container">
                <div className="card-bg-glow top-right blue" style={{ width: '400px', height: '400px', filter: 'blur(120px)', opacity: 0.15 }} />
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        {range === 'Daily' || range === 'Growth' ? (
                            ...
                            <BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                <CartesianGrid strokeDasharray="12 12" stroke="rgba(255,255,255,0.02)" vertical={false} />
                                <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#475569', fontSize: 10, fontWeight: 800 }} dy={15} />
                                <YAxis axisLine={false} tickLine={false} tick={{ fill: '#475569', fontSize: 10, fontWeight: 800 }} tickFormatter={v => `$${v}`} />
                        ...
                        <div key={i} className="day-col">
                            <div className="bar-container">
                                <div
                                    ...
                                    style={{ height: `${Math.min(100, (Math.abs(d.pnl || 0) / (bestDay || 1)) * 100)}%` }}
