...
const UserDetailsTab = observer(({ accountType }: { accountType: 'Real' | 'Demo' }) => {
    ...
    const portfolioTotal = Object.entries(client?.all_accounts_balance || {}).reduce((sum: number, [id, data]: [string, { balance?: number }]) => {
        const acc = accounts.find(a => a.loginid === id);
        if (acc && ((accountType === 'Demo' && acc.is_virtual) || (accountType === 'Real' && !acc.is_virtual))) {
            return sum + (data.balance || 0);
        }
        return sum;
    }, 0) || filtered.reduce((sum, a) => sum + (parseFloat((a as { balance?: string }).balance || '0') || 0), 0);
        ...
        <div className="user-details-panel">
            <div className="portfolio-summary">
                ...
                <div className="summary-info">
                    ...
                    <div className="balance-row">
                        <h2 className="total-balance">
                            {hideBalances ? '••••••' : `${client?.currency || 'USD'} ${portfolioTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
                        </h2>
                ...
                <div className="summary-actions">
                    <button onClick={() => setHideBalances(!hideBalances)} className="action-btn" title={hideBalances ? "Show" : "Hide"}>
            ...
            <div className="accounts-list">
                {loading ? (
                    Array(Math.max(filtered.length, 2)).fill(0).map((_, i) => (
                                    ...
                                    <div className="account-finances">
                                        <div className="finance-block">
                                            ...
                                            <h3 className="finance-value">
                                                {hideBalances ? '••••' : `${account.currency || 'USD'} ${balance.toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
                                            </h3>
                                        </div>
                                        <div className="finance-block">
